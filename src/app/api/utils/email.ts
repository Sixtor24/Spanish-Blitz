...
export const appBaseUrl = process.env.APP_BASE_URL || 'http://localhost:4000';
...
export async function sendEmail({ to, subject, html }: { to: string; subject: string; html: string }) {
  ...
  try {
    ...
    console.info('[email] send attempt', { to, subject, id: result?.data?.id, error: result?.error });
  } catch (err) {
    console.error('[email] Failed to send', err);
  }
}

export function premiumActivatedTemplate({ name, email }: { name?: string | null; email: string }) {
  ...
  return `
    <div style="font-family: Inter, system-ui, -apple-system, sans-serif; max-width: 520px; margin: 0 auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 12px;">
      ...
      <a href="${appBaseUrl}" style="display:inline-block;margin-top:16px;padding:12px 16px;background:#2563eb;color:#fff;text-decoration:none;border-radius:10px;font-weight:600;">Ir al dashboard</a>
